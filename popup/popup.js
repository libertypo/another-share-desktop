...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const getShareUrl = (id, t, u, q) => {
        ...
        if (id === 'share-custom' && data.customTemplate) {
            return buildCustomShareUrl(
                data.customTemplate,
                encodeURIComponent(u),
                encodeURIComponent(q ? `${RESERVED_INTRO}"${q}" — ${t}` : `${RESERVED_INTRO}${t}`)
            );
        }
    ...
    btnSplitStart.addEventListener('click', async () => {
        ...
        if (chunk.type === 'intent') {
            ...
            if (!isAllowedShareTarget(shareUrl, { allowMailto })) {
                Logger.warn(`Blocked unsafe thread share URL for platform: ${threadPlatform}`);
                showToast("Blocked unsafe share URL", "error");
                return;
            }
    ...
    order.forEach(id => {
        ...
        btn.addEventListener('click', async () => {
            ...
            const allowMailto = id === 'share-email';
            if (!isAllowedShareTarget(url, { allowMailto })) {
                Logger.warn(`Blocked unsafe share URL for platform: ${id}`);
                showToast("Blocked unsafe share URL", "error");
                return;
            }
