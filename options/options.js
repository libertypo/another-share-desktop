...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const validateCustomTemplate = (template) => {
        ...
        const testUrl = raw.replace('{url}', 'https://example.com').replace('{title}', 'test');
        try {
    ...
    document.getElementById('save-template').addEventListener('click', () => {
        ...
        if (template && !validateCustomTemplate(template)) {
            showStatus('Invalid template. Must be a valid URL with {url} and/or {title} placeholders.');
            return;
        }
